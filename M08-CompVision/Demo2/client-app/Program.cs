...
namespace VideoIndexerArm
{
    public class Program
    {
        ...
        static string apiKey; // replace with API key taken from https://aka.ms/viapi
        static string accountId; // replace with your VI account id (guid)
        static string accountLocation = "trial"; // If you have a paid account the location will be different, named after the Azure region the account is in
        static string videoUrl;  // Azure stoge best choise
        static string videoName; //Must be unique value for your subscription
        ...
        public static async Task Main(string[] args)
        {
            ...
            accountId = configuration.GetSection("accountId").Value;
            videoUrl = configuration.GetSection("videoUrl").Value;
            videoName = configuration.GetSection("videoName").Value;
           ...
           var client = new HttpClient(httpHandler);
            ...
            queryParams = CreateQueryString(
                new Dictionary<string, string>()
                {
                    {"name", videoName},
                    {"videoUrl", videoUrl},
                    {"description", "demo from code AI-102"},
